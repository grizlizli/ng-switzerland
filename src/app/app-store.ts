...
import { AI_PROVIDER_OPTIONS, type AiProviderId } from './features/ai/ai-provider';
...
export class AppStore {
  ...
  readonly #selection = inject(AI_PROVIDER_SELECTION);
  ...
  readonly providerField = form(this.#selection);
  readonly promptField = form(this.#prompt);
  readonly messages = this.#messages.asReadonly();
  readonly pending = this.#pending.asReadonly();
  readonly error = this.#error.asReadonly();
  readonly canSend = computed(() => !this.#pending() && this.#prompt().trim().length > 0);
  ...
  async send(): Promise<void> {
    ...
    try {
      ...
      this.#messages.update((messages) => [
        ...messages,
        { id: this.#nextMessageId++, provider, prompt, response },
      ]);
